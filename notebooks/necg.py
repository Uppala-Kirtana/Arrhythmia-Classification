...
from NeuralPCI import NeuralPCI
import os
...
def get_base_model_neural(in_channels, args=None):
    """
    Returns the model from the paper: Personalized Monitoring and Advance Warning System for Cardiac Arrhythmias
    with NeuralPCI integrated.
    """
    dim_pc = args['neuralpci_args'].get('dim_pc', None) if 'neuralpci_args' in args else None

    neuralpci_args = args.get('neuralpci_args', {})
    ...
    if dim_pc:
        neuralpci_model = NeuralPCI(in_channels=dim_pc, args=neuralpci_args)
        model = nn.Sequential(model, neuralpci_model)
    else:
        output_layer = nn.Linear(32, 2, bias=True)  # Change the output layer for the neural model
...
def get_performance_metrics(cm):
    ...
    """
    cm = np.asarray(cm).copy().astype(np.int64)
...
def get_confusion_matrix(pred_y, true_y, pos_is_zero=False):
    ...
    true_y = torch.as_tensor(true_y, dtype=torch.long).reshape(-1)
    vals = true_y + 2 * pred_y   # 0,0 -> 0    1,0 -> 1    0,1 -> 2    1,1 -> 3

    # Move the vals tensor to the same device as pred_y and true_y
    vals = vals.to(pred_y.device)

    cm = torch.zeros(4, device=pred_y.device).long()
